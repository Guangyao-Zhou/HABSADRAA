...
from att_layer import bilinear_attention_layer, dot_produce_attention_layer,  match_attention_layer
from config import *
...
def lcr_rot(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, keep_prob1, keep_prob2, l2, _id='all'):
    print('I am a new lcr_rot_alt. Trival2021 model 3')
    ...
    outputs_t_l_1 = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs_1[:,:,1], 2), outputs_t_l_init))
    ...
    #outputs_r = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,1], 2), outputs_r_init))
    outputs_t_r_1 = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs_2[:,:,1], 2), outputs_t_r_init))
    #outputs_t_r = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,3], 2), outputs_t_r_init))
    ...
    for i in range(1):        
        # attention left
        att_t_l = bilinear_attention_layer(hiddens_t, outputs_t_l_1, sen_len_tr, 2 * FLAGS.n_hidden, l2, FLAGS.random_base, 'l'+str(i))
        outputs_l_init = tf.matmul(att_t_l, hiddens_t)
        outputs_l = tf.squeeze(outputs_l_init)
                                                       
        # attention right
        att_t_r = bilinear_attention_layer(hiddens_t, outputs_t_r_1, sen_len_tr, 2 * FLAGS.n_hidden, l2, FLAGS.random_base, 'r'+str(i))
        outputs_r_init = tf.matmul(att_t_r, hiddens_t)
        outputs_r = tf.squeeze(outputs_r_init) 
...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=0.09, keep_prob=0.3, momentum=0.85, l2=0.00001):
    ...
    with tf.device('/gpu:1'):
        ...
        #word_id_mapping, w2v = load_w2v(FLAGS.embedding_path, 768)
