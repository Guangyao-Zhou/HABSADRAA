...
def match_attention_layer(inputs, inputs2, attend, length, n_hidden, l2_reg, random_base, layer_id=1):
    """
    :param inputs: batch * max_len * n_hidden
    :param attend: batch * max_len_b * n_hidden
    :param length:
    :param n_hidden:
    :param l2_reg:
    :param random_base:
    ...
    '''
    inputs = tf.reshape(inputs, [max_len, n_hidden])
    tmp = tf.reshape(tf.matmul(inputs, w), [-1, max_len, n_hidden])
    attend = tf.expand_dims(attend, 2)
